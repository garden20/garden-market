...
exports.showInstallChoices = function(req) {
    ...
    var loc = window.location.toString();
    loc = loc.substring(0, loc.length -  8); // remove the trailing /install
    var g20 = 'http://garden20.com/?app_url=' + escape(loc);
    $('.g20').attr('href', g20);
    ...
    appdb.getView('garden', 'hosting', { include_docs: true }, function (err, data) {
        ...
        $('#hosting_providers ul').append(
            templates.render('own_garden_option.html', req, {
                dashboard_urls: _.map(data.rows, function (row, i) {
                    return {
                        id: i,
                        url: row.doc.url + '?app_url=' + escape(loc),
